...
void setup_pressure_sensor(){
    ...
    mpr0.begin();
    if (!mpr0.begin()) {
        Serial.println("Pressure sensor 0 is not ready.");
        while(1);
    }
    ...
    mpr1.begin();
    if (!mpr1.begin()) {
        Serial.println("Pressure sensor 1 is not ready.");
        while(1);
    }
    ...
    mpr2.begin();
    if (!mpr2.begin()) {
        Serial.println("Pressure sensor 2 is not ready.");
        while(1);
    }
    ...
    mpr3.begin();
    if (!mpr3.begin()) {
        Serial.println("Pressure sensor 3 is not ready.");
        while(1);
    }
}
